...
- spdlog-like API: `logger`, `sinks`, `pattern_formatter`, `level`, `registry`,
  and the `QUILL_LOGGER_*`/`QUILL_*` logging macros.
- Compile-time format string checking (`std::format_string`) and compile-time
  level gating (`QUILL_ACTIVE_LEVEL`); `log_runtime`/`log_raw` escape hatches.
...
- CMake presets (`dev`/`debug`/`release`/`relwithdebinfo`/`bench`/`ci`/
  `asan`/`tsan`/`coverage`) and install/export (`find_package(quill CONFIG)`).
...
- `std::format` by default with an opt-in `fmt` fallback (`QUILL_USE_STD_FORMAT`).

[Unreleased]: https://github.com/Nekoycode/quill/compare/v0.1.0...HEAD
[0.1.0]: https://github.com/Nekoycode/quill/releases/tag/v0.1.0
