...
#define QUILL_LOGGER_CALL(logger, lvl, ...)                                      \
  ...
  #define QUILL_LOGGER_TRACE(logger, ...)                                        \
    QUILL_LOGGER_CALL(logger, quill::level::trace, __VA_ARGS__)
#else
  #define QUILL_LOGGER_TRACE(logger, ...) ((void)0)
#endif

#if QUILL_ACTIVE_LEVEL <= QUILL_LEVEL_DEBUG
  #define QUILL_LOGGER_DEBUG(logger, ...)                                        \
    QUILL_LOGGER_CALL(logger, quill::level::debug, __VA_ARGS__)
#else
  #define QUILL_LOGGER_DEBUG(logger, ...) ((void)0)
#endif

#if QUILL_ACTIVE_LEVEL <= QUILL_LEVEL_INFO
  #define QUILL_LOGGER_INFO(logger, ...)                                         \
    QUILL_LOGGER_CALL(logger, quill::level::info, __VA_ARGS__)
#else
  #define QUILL_LOGGER_INFO(logger, ...) ((void)0)
#endif

#if QUILL_ACTIVE_LEVEL <= QUILL_LEVEL_WARN
  #define QUILL_LOGGER_WARN(logger, ...)                                         \
    QUILL_LOGGER_CALL(logger, quill::level::warn, __VA_ARGS__)
#else
  #define QUILL_LOGGER_WARN(logger, ...) ((void)0)
#endif

#if QUILL_ACTIVE_LEVEL <= QUILL_LEVEL_ERROR
  #define QUILL_LOGGER_ERROR(logger, ...)                                        \
    QUILL_LOGGER_CALL(logger, quill::level::error, __VA_ARGS__)
#else
...
#define QUILL_CRITICAL(...)                                                      \
  QUILL_LOGGER_CRITICAL(quill::default_logger(), __VA_ARGS__)
...
inline std::shared_ptr<sinks::sink> basic_file_sink(std::string filename,
                                                    bool truncate = false) {
  return std::make_shared<sinks::basic_file_sink>(std::move(filename), truncate);
}

inline std::shared_ptr<sinks::sink> rotating_file_sink(std::string filename,
                                                       std::size_t max_size,
                                                       std::size_t max_files) {
  return std::make_shared<sinks::rotating_file_sink>(std::move(filename), max_size,
                                                     max_files);
}

inline std::shared_ptr<sinks::sink> daily_file_sink(std::string filename,
                                                    int rotation_hour = 0,
...
template <typename... Sinks>
std::shared_ptr<async_logger> create_async_logger(std::string name,
...
inline std::shared_ptr<logger> file_logger(const std::string& name,
